...
docker run -d --name mysql57 --net=backend -p 3306:3306 -e MYSQL_ROOT_PASSWORD=pts123456. --restart=always -v /opt/meeting-server/mysql/data:/var/lib/mysql -v /opt/meeting-server/mysql/config/mysqld.cnf:/etc/mysql/conf.d/mysqld.cnf -v /opt/meeting-server/mysql/sql:/docker-entrypoint-initdb.d hub.c.163.com/library/mysql:5.7
